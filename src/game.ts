...
import { TICK_RATE, Update,Select } from "../start_server2";
class Game {
    ...
    Done(state: number = 0, worker: any): void {
        ...
        if (state == 0) {
            ...
            this.players.forEach((player) => {
                Select(player.id, undefined, ['coin']).then((res: any) => {
                    console.log(player.name,res,this.score.get(player.id));
                    Update(player.id, ["coin"], [this.score.get(player.id) + res.coin]);
                })
